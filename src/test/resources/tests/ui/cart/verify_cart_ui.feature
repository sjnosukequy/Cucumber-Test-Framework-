Feature: Verify Cart Functionality
  ...
  Scenario: Verify user sees empty cart message and no cart table when cart has no items
    ...
    And the cart product table should not be displayed - "cart"
  ...
  Scenario Outline: Verify user sees required product details for a single cart item
    ...
    Then the cart product table should be displayed - "cart"
    And the cart row for "<product>" should be displayed - "cart"
    And the product image, name, price, and quantity should be visible and correct - "cart"
    And the line total should be displayed correctly as price multiplied by quantity - "cart"
    And the delete icon should be displayed for the item - "cart"
  ...
  Scenario Outline: Verify user can remove a selected item from cart with multiple items
    ...
    When I open the Cart page from the header
    Then the cart product table should be displayed - "cart"
    When I delete the product "<deletedProduct>" from the cart - "cart"
    Then the deleted items should be removed from the cart - "cart"
    And the non-deleted items should remain displayed correctly - "cart"
    And the line total should be displayed correctly as price multiplied by quantity - "cart"
  ...
  Scenario Outline: Verify user sees empty cart state after deleting the final item
    ...
    Then the cart product table should be displayed - "cart"
    And the cart row for "<product>" should be displayed - "cart"
    When I delete the product "<product>" from the cart - "cart"
    Then the deleted items should be removed from the cart - "cart"
    And the cart product table should not be displayed - "cart"
    And the cart checkout button should be hidden
    And an empty cart message with a continue shopping prompt should be displayed
